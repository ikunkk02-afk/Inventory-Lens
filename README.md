...
第二阶段：准星指向 6 格内的普通箱子、大箱子或木桶时，通过服务端验证并同步只读库存快照，绘制贴在当前观察面下半部的原版浅灰 9×3 / 9×6 小面板。以表面为基准，最多向摄像机转动 12°，保留有限 Billboard 跟随与透视感；正面、侧面、背面均适用，超出显示视角时隐藏。
顶部预览平贴在箱盖中心，位置、方向和大小均绑定箱子；转头、绕行或重新瞄准都不会改变姿态。
不打开菜单，不允许移动物品。完整说明、验证边界和 A–L 测试清单见 [第二阶段说明](docs/phase-two-containers.md)。
...
装备面板仍只依赖客户端。容器投影要求客户端和服务端都安装此版本及 Fabric API；单人游戏使用模组自带的集成服务端逻辑。
服务器没有快照通道时自动停用容器投影，装备面板继续工作。
...
- 装备面板暂无 Tooltip、配置、动画、玩家装备或特殊 Boss 逻辑。
...
当前版本新增箱子、大箱子和木桶的只读容器投影；第一阶段的装备布局、避障算法和 Glint 渲染路径保持原有行为。
